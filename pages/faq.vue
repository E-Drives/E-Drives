<template>
  <v-container>
    ...
    <v-row v-for="(item, index) in items" :key="item.question">
      <v-col>
        <p class="title secondary--text">{{ index + 1 }}. {{ item.question }}</p>
        <v-container fluid>
          <p class="body-1" v-for="(answer, index) in item.answers" :key="index">{{ answer }}</p>
        </v-container>
      </v-col>
    </v-row>
  </v-container>
...
export default {
  ...
  data: () => ({
    items: [
      {
        question: "What is the minimum Age to book E Drives?",
        answers: [
          "The user must be above the age of 18 years. However, certain vehicles are provided at the minimum age of 18."
        ]
      },
      {
        question: "What all documents required at the time of booking?",
        answers: [
      ...
      },
      {
        ...
        answers: ["E Drives does not charge any security deposit for bookings."]
      },
      {
        question: "From where should i Pickup the vehicle?",
        answers: [
      ...
      {
        question: "Where do I need to return the vehicle?",
        answers: [
      ...
      {
        question: "How to I charge the vehicle?",
        answers: [
      ...
      {
        question: "Is there any Distance limit?",
        answers: ["There is no Cap on Distance travelled."]
      },
      {
        question: "What all the Penalty charges?",
        answers: [
          ...
          "7. In the event of any traffic rule violation or parking fine, the member must inform EDrives immediately. EDrives may ask for a copy of the challan in such an event.",
          ...
          "10. In the event of user having any penalties/dues outstanding with EDrives, EDrives reserves the right to charge reminder fees and default interest. EDrives also reserves the right to utilize third parties for the recovery of dues from a user.",
          "11. EDrives reserves the right to prohibit a user from booking any rides until all dues associated with the user account have been paid to EDrives in full.",
          "12. The user would be charged with a penalty in case any accessory or documents, belonging to EDrives is found missing during the end ride.",
          "13. Damage charges apply in case of damage to the vehicle, it depends on the severity of the damage. However, the maximum liability that the user is liable to pay in case of a major damage is Rs. 9999 (subject to certain T&Cs)."
        ]
      },
      {
        question: "IS the billing amount inclusive of GST?",
        answers: [
      ...
      {
        question: "Does E Drives Provide Helmet?",
        answers: [
      ...
      {
        question: "What is the Threshold Time?",
        answers: [
